...
hk18DC <-
  st_read("DC_2015_poly Shapefile/")
...
w %>% st_as_sf(data.frame(n=1:length(.))) %>% mapview() ### linestring not in proper order
...
wg<-w %>%
  ...
  as_tbl_graph() %>%
  mutate(m=group_components(), n=1:length(w),
         startpt=(w %>%
                    st_startpoint() %>%
...
ww<-w %>% 
  st_intersects(w, sparse=F) %>%
  magrittr::and(.,upper.tri(.)) %>%  ## avoid self intersects
  igraph::graph_from_adjacency_matrix(mode="undirected") %>%
  as_tbl_graph() %>%
  mutate(m=group_components(), n=1:length(w),geometry=w)
